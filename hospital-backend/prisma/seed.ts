...
// Pakistani first/last names for generating patients
const maleFirstNames = [
  'Ahmed', 'Muhammad', 'Ali', 'Hassan', 'Usman', 'Bilal', 'Fahad', 'Saad',
  'Omar', 'Imran', 'Kamran', 'Tariq', 'Naveed', 'Khalid', 'Shahid', 'Asif',
  'Waseem', 'Rizwan', 'Junaid', 'Faisal', 'Zubair', 'Hamza', 'Aamir', 'Nadeem',
  'Adeel', 'Kashif', 'Waqar', 'Sajid', 'Rehan', 'Danish', 'Irfan', 'Shoaib',
  'Tahir', 'Zeeshan', 'Atif', 'Arif', 'Sohail', 'Mazhar', 'Nasir', 'Yasir',
];

const femaleFirstNames = [
  'Fatima', 'Ayesha', 'Zainab', 'Sana', 'Hira', 'Maryam', 'Nadia', 'Sara',
  'Rabia', 'Amina', 'Khadija', 'Sumaya', 'Noor', 'Iqra', 'Bushra', 'Samina',
  'Shabana', 'Rubina', 'Nasreen', 'Farida', 'Tahira', 'Saima', 'Uzma', 'Reshma',
  'Asma', 'Sidra', 'Mehwish', 'Fariha', 'Nargis', 'Parveen', 'Riffat', 'Sajida',
  'Shazia', 'Naheed', 'Ghazala', 'Fouzia', 'Rukhsana', 'Shaista', 'Anila', 'Lubna',
];

const lastNames = [
  'Khan', 'Ali', 'Ahmed', 'Malik', 'Sheikh', 'Hussain', 'Raza', 'Shah',
  'Butt', 'Awan', 'Siddiqui', 'Chaudhry', 'Qureshi', 'Iqbal', 'Aslam',
  'Mirza', 'Hashmi', 'Gilani', 'Abbasi', 'Naqvi', 'Rizvi', 'Javed',
  'Farooq', 'Mehmood', 'Nasir', 'Yousaf', 'Baig', 'Rajput', 'Mughal', 'Lodhi',
];
...
function generatePatientEmail(): string {
  patientEmailCounter++;
  return `patient.seed${patientEmailCounter}@hospital.com`;
}

function generateCnic(index: number): string {
  const area = 35000 + (index % 10000);
  const mid = 1000000 + index;
  const last = index % 10;
...
function generatePhone(index: number): string {
  ...
  return `+92-${prefix[index % prefix.length]}-${(1000000 + index).toString().slice(0, 7)}`;
}
...
function generateDateOfBirth(index: number): Date {
  ...
  const age = 5 + Math.floor(seededRandom(index + 100) * 70);
  const d = new Date(2026, 2, 1); // March 1, 2026
  d.setFullYear(d.getFullYear() - age);
  d.setMonth(Math.floor(seededRandom(index + 200) * 12));
  d.setDate(1 + Math.floor(seededRandom(index + 300) * 27));
  return d;
}
...
function generateCheckupDate(csvIndex: number): Date {
  ...
  const offset = seededRandom(csvIndex * 7 + 42) * range;
  const d = new Date(startDate.getTime() + offset);
  // Set a realistic hour between 9 AM and 8 PM
  d.setHours(9 + Math.floor(seededRandom(csvIndex * 3 + 11) * 11));
  d.setMinutes(Math.floor(seededRandom(csvIndex * 5 + 17) * 60));
  d.setSeconds(0, 0);
  return d;
}
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║         Hospital Database Seed - Extended (100+)         ║');
  console.log('╚════════════════════════════════════════════════════════════╝\n');
  ...
  // Map from CSV department_name -> Department record
  ...
  for (const dc of deptConfigs) {
    ...
    // Map by short name (CSV uses e.g. "Cardiology")
  ...
  // ===========================
  // STEP 6: Doctors (one per department)
  // ===========================
  console.log('👨‍⚕️ Creating doctors...');
  const doctorConfigs = [
    { email: 'doctor@hospital.com', firstName: 'Dr. Ali', lastName: 'Hassan', cnic: '42101-2345678-1', gender: 'MALE' as const, dept: 'Cardiology', license: 'PMC-12345', spec: 'Interventional Cardiology', exp: 15, qual: 'MBBS, FCPS (Cardiology)' },
    { email: 'dr.fatima@hospital.com', firstName: 'Dr. Fatima', lastName: 'Khan', cnic: '42201-3456789-2', gender: 'FEMALE' as const, dept: 'Gynecology', license: 'PMC-23456', spec: 'Obstetrics and Gynecology', exp: 12, qual: 'MBBS, FCPS (Gynecology)' },
    { email: 'dr.ahmed@hospital.com', firstName: 'Dr. Ahmed', lastName: 'Malik', cnic: '42301-4567890-3', gender: 'MALE' as const, dept: 'Neurology', license: 'PMC-34567', spec: 'Neurologist', exp: 18, qual: 'MBBS, FCPS (Neurology)' },
    { email: 'dr.sara@hospital.com', firstName: 'Dr. Sara', lastName: 'Ahmad', cnic: '42401-5678901-4', gender: 'FEMALE' as const, dept: 'Pediatrics', license: 'PMC-45678', spec: 'Pediatrician', exp: 10, qual: 'MBBS, DCH, FCPS (Pediatrics)' },
    { email: 'dr.usman@hospital.com', firstName: 'Dr. Usman', lastName: 'Sheikh', cnic: '42501-6789012-5', gender: 'MALE' as const, dept: 'Orthopedics', license: 'PMC-56789', spec: 'Orthopedic Surgeon', exp: 20, qual: 'MBBS, FCPS (Orthopedics)' },
    { email: 'dr.kamal@hospital.com', firstName: 'Dr. Kamal', lastName: 'Rashid', cnic: '42601-7890123-6', gender: 'MALE' as const, dept: 'General Medicine', license: 'PMC-67890', spec: 'General Physician', exp: 14, qual: 'MBBS, FCPS (Medicine)' },
    { email: 'dr.nadia@hospital.com', firstName: 'Dr. Nadia', lastName: 'Yasmeen', cnic: '42701-8901234-7', gender: 'FEMALE' as const, dept: 'Radiology', license: 'PMC-78901', spec: 'Radiologist', exp: 11, qual: 'MBBS, FCPS (Radiology)' },
  ];

  // dept name -> Doctor record
  ...
  const receptionistConfigs = [
    { email: 'receptionist@hospital.com', firstName: 'Sarah', lastName: 'Khan', cnic: '42201-9876543-2', gender: 'FEMALE' as const, phone: '+92-321-9876543', exp: 5, qual: 'Diploma in Healthcare Administration' },
    { email: 'ali.reception@hospital.com', firstName: 'Ali', lastName: 'Raza', cnic: '42301-8765432-1', gender: 'MALE' as const, phone: '+92-333-8765432', exp: 3, qual: 'Bachelor in Business Administration' },
    { email: 'maria.reception@hospital.com', firstName: 'Maria', lastName: 'Siddiqui', cnic: '42401-7654321-0', gender: 'FEMALE' as const, phone: '+92-311-7654321', exp: 7, qual: 'Diploma in Medical Office Administration' },
  ];
  ...
  const labTechConfigs = [
    { email: 'labtechnician@hospital.com', firstName: 'Hamza', lastName: 'Sheikh', cnic: '42501-1122334-4', gender: 'MALE' as const, dept: 'Cardiology', spec: 'Clinical Chemistry', exp: 8, qual: 'BS Medical Laboratory Technology' },
    { email: 'zahid.labtech@hospital.com', firstName: 'Zahid', lastName: 'Mahmood', cnic: '42601-2233445-5', gender: 'MALE' as const, dept: 'Neurology', spec: 'Hematology', exp: 6, qual: 'BS Medical Laboratory Technology' },
    { email: 'fatima.labtech@hospital.com', firstName: 'Fatima', lastName: 'Noor', cnic: '42701-3344556-6', gender: 'FEMALE' as const, dept: 'Orthopedics', spec: 'Microbiology', exp: 5, qual: 'BS Microbiology' },
    { email: 'ayesha.labtech@hospital.com', firstName: 'Ayesha', lastName: 'Zafar', cnic: '42801-4455667-7', gender: 'FEMALE' as const, dept: 'Pediatrics', spec: 'Immunology', exp: 4, qual: 'BS Medical Laboratory Technology' },
  ];
  ...
  const pathConfigs = [
    { email: 'pathologist@hospital.com', firstName: 'Dr. Imran', lastName: 'Qureshi', cnic: '42901-5566778-8', gender: 'MALE' as const, dept: 'Cardiology', spec: 'Clinical Pathology', exp: 12, qual: 'MBBS, MPhil Pathology' },
    { email: 'dr.sadia.path@hospital.com', firstName: 'Dr. Sadia', lastName: 'Akhtar', cnic: '42102-6677889-9', gender: 'FEMALE' as const, dept: 'Neurology', spec: 'Histopathology', exp: 10, qual: 'MBBS, FCPS Histopathology' },
    { email: 'dr.kashif.path@hospital.com', firstName: 'Dr. Kashif', lastName: 'Iqbal', cnic: '42202-7788990-0', gender: 'MALE' as const, dept: 'Orthopedics', spec: 'Chemical Pathology', exp: 15, qual: 'MBBS, FCPS Chemical Pathology' },
    { email: 'dr.nida.path@hospital.com', firstName: 'Dr. Nida', lastName: 'Hassan', cnic: '42302-8899001-1', gender: 'FEMALE' as const, dept: 'Gynecology', spec: 'Cytopathology', exp: 9, qual: 'MBBS, MPhil Pathology' },
  ];
  ...
  // ===========================
  // STEP 10: Drugs (comprehensive list)
  // ===========================
  ...
  const templateDefs = [
    ...
    // Additional templates for CSV tests
    {
  ...
  const csvPath = path.resolve(__dirname, '..', 'checkup_data.csv');
  ...
  const csvPatientIds: number[] = []; // index -> patient.id
  ...
  for (let i = 0; i < csvRows.length; i++) {
    ...
    const patient = await prisma.patient.create({
      data: {
        ...
        emergencyContact: generatePhone(i + 500),
        onboardingDone: true,
  ...
  // Schedules from Jan 1 2026 to Mar 5 2026
  ...
  // Track slot usage per doctor per date
  ...
  for (let i = 0; i < csvRows.length; i++) {
    ...
    // Determine department and doctor
    ...
    // Generate a date for this checkup
    ...
    // Create appointment
    const isWalkin = i % 3 === 0; // ~1/3 are walk-ins
