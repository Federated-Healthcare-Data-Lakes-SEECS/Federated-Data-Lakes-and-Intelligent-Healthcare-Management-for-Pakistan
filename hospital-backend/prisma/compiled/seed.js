...
async function main() {
    ...
    async function createDoctorScheduleWithSlots(doctor, scheduleDate) {
        // Skip Sunday (day 0)
    ...
    async function createAppointmentWithCheckup(patient, slot, reason, type, receptionist, checkupData, status = 'BOOKED') {
    ...
    if (primaryDoctor && primaryPatient && doctorSlots['doctor@hospital.com'][-2]) {
        ...
        if (slots) {
            await createAppointmentWithCheckup(primaryPatient, slots.morningSlots[0], 'Chest pain and high blood pressure concerns', 'online', null, {
    ...
    if (primaryDoctor && primaryPatient && doctorSlots['doctor@hospital.com'][-1]) {
        ...
        if (slots) {
            await createAppointmentWithCheckup(primaryPatient, slots.morningSlots[1], 'Follow-up - Lab results and BP monitoring', 'walkin', primaryReceptionist, {
    ...
    if (primaryDoctor && primaryPatient && doctorSlots['doctor@hospital.com'][0]) {
        ...
        if (slots) {
            await createAppointmentWithCheckup(primaryPatient, slots.eveningSlots[2], 'Routine follow-up - BP and medication review', 'online', null, null, 'BOOKED');
        }
    }
    ...
    if (primaryDoctor && primaryPatient && doctorSlots['doctor@hospital.com'][2]) {
        const slots = doctorSlots['doctor@hospital.com'][2];
        if (slots) {
            await createAppointmentWithCheckup(primaryPatient, slots.morningSlots[3], 'Follow-up after 2 weeks of combined therapy', 'online', null, null, 'BOOKED');
        }
    }
    // =========================================================
    ...
    if (primaryDoctor && patientUmar && doctorSlots['doctor@hospital.com'][-2]) {
        const slots = doctorSlots['doctor@hospital.com'][-2];
        if (slots) {
            await createAppointmentWithCheckup(patientUmar, slots.morningSlots[2], 'High cholesterol follow-up, family history of heart disease', 'walkin', primaryReceptionist, {
    ...
    if (primaryDoctor && patientBilal && doctorSlots['doctor@hospital.com'][-1]) {
        const slots = doctorSlots['doctor@hospital.com'][-1];
        if (slots) {
            await createAppointmentWithCheckup(patientBilal, slots.eveningSlots[0], 'Cardiac screening - diabetic patient with family history of heart attack', 'online', null, {
    ...
    if (primaryDoctor && patientSana && doctorSlots['doctor@hospital.com'][1]) {
        const slots = doctorSlots['doctor@hospital.com'][1];
        if (slots) {
            await createAppointmentWithCheckup(patientSana, slots.morningSlots[0], 'Palpitations and occasional dizziness - needs cardiac evaluation', 'online', null, null, 'BOOKED');
        }
    }
    // =========================================================
    ...
    if (neurologist && patientSana && doctorSlots['dr.ahmed@hospital.com'][-1] && primaryReceptionist) {
        ...
        if (slots) {
            await createAppointmentWithCheckup(patientSana, slots.morningSlots[1], 'Severe migraine episode with visual aura', 'walkin', primaryReceptionist, {
    ...
    if (orthopedic && patientHassan && doctorSlots['dr.usman@hospital.com'][-2] && primaryReceptionist) {
        ...
        if (slots) {
            await createAppointmentWithCheckup(patientHassan, slots.eveningSlots[1], 'Post ACL surgery follow-up - 6 month review', 'walkin', primaryReceptionist, {
    ...
    if (gynecologist && patientZainab && doctorSlots['dr.fatima@hospital.com'][0] && primaryReceptionist) {
        ...
        if (slots) {
            await createAppointmentWithCheckup(patientZainab, slots.morningSlots[2], 'Routine gynecological checkup', 'walkin', primaryReceptionist, null, 'BOOKED');
        }
    }
    ...
    if (pediatrician && patientAyesha && doctorSlots['dr.sara@hospital.com'][1] && primaryReceptionist) {
        ...
        if (slots) {
            await createAppointmentWithCheckup(patientAyesha, slots.morningSlots[0], 'Child vaccination and routine checkup', 'walkin', primaryReceptionist, null, 'BOOKED');
        }
    }
    // =========================================================
    ...
    if (gynecologist && patientZainab && doctorSlots['dr.fatima@hospital.com'][-2]) {
        ...
        if (slots) {
            await createAppointmentWithCheckup(patientZainab, slots.morningSlots[0], 'Irregular menstrual cycles and hormonal concerns', 'online', null, {
    ...
    if (pediatrician && patientAyesha && doctorSlots['dr.sara@hospital.com'][-1]) {
        const slots = doctorSlots['dr.sara@hospital.com'][-1];
        if (slots) {
            await createAppointmentWithCheckup(patientAyesha, slots.eveningSlots[2], 'Asthma management consultation for self and child', 'online', null, {
    ...
    if (neurologist && patientBilal && doctorSlots['dr.ahmed@hospital.com'][-2]) {
        ...
        if (slots) {
            await createAppointmentWithCheckup(patientBilal, slots.morningSlots[3], 'Tingling sensations in hands, diabetic neuropathy concern', 'online', null, {
    ...
    if (orthopedic && patientUmar && doctorSlots['dr.usman@hospital.com'][-1]) {
        ...
        if (slots) {
            await createAppointmentWithCheckup(patientUmar, slots.morningSlots[2], 'Lower back pain radiating to leg', 'walkin', allReceptionists.find(r => r.user.email === 'ali.reception@hospital.com') || primaryReceptionist, {
    ...
    if (primaryDoctor && patientUmar && doctorSlots['doctor@hospital.com'][3]) {
        ...
        if (slots) {
            await createAppointmentWithCheckup(patientUmar, slots.eveningSlots[0], 'Lipid panel review and medication adjustment', 'online', null, null, 'BOOKED');
        }
    }
    // Day +4 appointment
    if (neurologist && patientSana && doctorSlots['dr.ahmed@hospital.com'][4]) {
        ...
        if (slots) {
            await createAppointmentWithCheckup(patientSana, slots.morningSlots[1], 'Migraine follow-up - prophylactic therapy review', 'online', null, null, 'BOOKED');
        }
    }
